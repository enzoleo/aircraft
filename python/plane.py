...
class EnemyLightPlane(EnemyPlane):
    ...
    def fire(self):
        if scipy.stats.bernoulli.rvs(0.01):
            # Initialize the bullet first to obtain its image size.
            enemy_bullet = EnemyNormalBullet(0, 0)
            w, _ = enemy_bullet.image.get_rect().size
            width, height = self.image.get_rect().size

             # Update the real location of the bullet.
            enemy_bullet.location.x = self.location.x + (width - w) * 0.5
            enemy_bullet.location.y = self.location.y + height
            AircraftWar.newcome.append(enemy_bullet)
